...
namespace Lokas.Editor.DataTableTools
{
    public sealed class DataTableGenerator
    {
        ...
        {
            string dataTableFilePath = Utility.Path.GetRegularPath(Path.Combine(DataTablePath, dataTableName + ".txt"));
            try
            {
                // Prefer UTF-8 for current tables while keeping backward compatibility.
                return new DataTableProcessor(dataTableFilePath, Utf8Encoding, 1, 2, null, 3, 4, 1);
            }
            catch (DecoderFallbackException)
            {
                return new DataTableProcessor(dataTableFilePath, Gb2312Encoding, 1, 2, null, 3, 4, 1);
            }
        }
        ...
        private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
        {
            ...
            codeContent.Replace("__DATA_TABLE_NAME_SPACE__", DataTableExtension.DataTableNamespace);
        ...
        private static string GenerateDataTableParser(DataTableProcessor dataTableProcessor)
        {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                if (dataTableProcessor.IsSystem(i))
                {
                    ...
                    {
                        stringBuilder.AppendFormat("            {0} = {1}.Parse(columnStrings[index++],CultureInfo.InvariantCulture);", dataTableProcessor.GetName(i), languageKeyword).AppendLine();
                    }
                }
                else
                {
