...
namespace UI.Pagination
{
    public partial class PagedRect
    {
        protected void RecalculateDesiredPageSizes()
        {
            if (ShowPagePreviews)
            {
                if (UseClippedPagePreviews)
                {
                    var currentScale = Mathf.Clamp(ClippedCurrentPageScale, 0.01f, 2f);
                    var previewScale = Mathf.Clamp(ClippedPagePreviewScale, 0.01f, 2f);

                    m_currentPageSize = new Vector3(sizingTransform.rect.width * currentScale, sizingTransform.rect.height * currentScale, 1);
                    m_otherPageSize = new Vector3(sizingTransform.rect.width * previewScale, sizingTransform.rect.height * previewScale, 1);

                    m_currentPageScale = new Vector3(currentScale, currentScale, 1);
                    m_otherPageScale = new Vector3(previewScale, previewScale, 1);
                    return;
                }
        ...
        public float GetPageOffset_PagePreviews(Page page)
        {
            if (UseClippedPagePreviews)
            {
                float pageSize = GetClippedPreviewPageSize();
                float viewportSize = GetClippedPreviewViewportSize();
                float currentPageSize = pageSize * Mathf.Clamp(ClippedCurrentPageScale, 0.01f, 2f);
                float previewPageSize = pageSize * Mathf.Clamp(ClippedPagePreviewScale, 0.01f, 2f);
                int clippedPagesBefore = GetPagePosition(page) - 1;
                float clippedDistanceBeforePage = clippedPagesBefore * (previewPageSize + SpaceBetweenPages);
                float clippedOffset = (viewportSize * 0.5f) - clippedDistanceBeforePage - (currentPageSize * 0.5f);

                return ScrollRect.horizontal ? clippedOffset : -clippedOffset;
            }
        ...
        private float GetClippedPreviewPageSize()
        {
            if (sizingTransform == null)
                return 0f;

            return ScrollRect.horizontal ? sizingTransform.rect.width : sizingTransform.rect.height;
        }

        private float GetClippedPreviewViewportSize()
        {
            RectTransform viewportTransform = null;

            if (ScrollRectViewport != null)
                viewportTransform = ScrollRectViewport.transform as RectTransform;

            if (viewportTransform == null && ScrollRect != null)
                viewportTransform = ScrollRect.transform as RectTransform;

            if (viewportTransform == null)
                return GetClippedPreviewPageSize();

            return ScrollRect.horizontal ? viewportTransform.rect.width : viewportTransform.rect.height;
        }
