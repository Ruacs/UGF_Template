...
namespace UI.Pagination
{
    public partial class PagedRect
    {
        ...
        protected Dictionary<int, float> GetPageDistancesFromScrollRectCenter()
        {
            ...
            float centerPosition = scrollRectPosition + (GetScrollRectViewportSize() * 0.5f);
            ...
            for (var x = 0; x < childCount; x++)
            {
                ...
                float scaledPageSize = pageSize * pageScale;

                pageDistances.Add(page.PageNumber, Mathf.Abs(centerPosition - basePosition - (scaledPageSize * 0.5f)));

                basePosition += scaledPageSize + SpaceBetweenPages;
            }

            return pageDistances;
        }

        private float GetScrollRectViewportSize()
        {
